...
class Borrow(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    book = db.Column(db.String(64), index=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    
    def __repr__(self):
        return '<Borrow {}>'.format(self.book)

class Book(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    book = db.Column(db.String(64), index=True, unique=True)
    author = db.Column(db.String(64), index=True)
    type = db.Column(db.String(64), index=True)
    availibility = db.Column(db.String(64), index=True)
